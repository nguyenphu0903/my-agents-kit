...
const SUPPORTED_PROVIDERS = ["codex", "copilot", "opencode", "claude", "gemini"];
        ...
        if (fs.existsSync(path.join(targetDir, ".git"))) {
            try {
                ...
                const localOnlyArtifacts = [
                    selectedProviders.has("codex") ? "AGENTS.md" : null,
                    selectedProviders.has("codex") ? ".codex/skills/" : null,
                    selectedProviders.has("codex") ? ".codex/agents/" : null,
...
function publishWorkspaceProviderAssets(sourceRoot, targetDir, providers) {
    if (providers.has("codex")) {
        publishCodexWorkspaceAssets(sourceRoot, targetDir);
    }
...
function publishGlobalProviderAssets(sourceRoot, providers) {
    if (providers.has("codex")) {
        publishCodexGlobalAssets(sourceRoot);
    }
...
function publishCodexWorkspaceAssets(sourceRoot, targetDir) {
    ...
    generateCodexSkills(sourceRoot, targetDir, true);
    generateCodexAgents(sourceRoot, targetDir, true);
}

function publishCodexGlobalAssets(sourceRoot) {
    ...
    generateCodexSkills(sourceRoot, os.homedir(), false);
    generateCodexAgents(sourceRoot, codexRoot, false);
}
...
function generateCodexSkills(sourceRoot, targetDir, useRepoPrefix = true) {
    const sourceSkillsDir = path.join(sourceRoot, KIT_DIR, "skills");
    ...
    const targetSkillsDir = useRepoPrefix
        ? path.join(targetDir, CODEX_SKILLS_DIR)
    ...
    for (const skillName of skillDirs) {
        ...
        copyManagedDirectory(
            ...
            path.join(KIT_DIR, "skills", skillName),
        );
        exportedCount += 1;
    }

    console.log(
        `âœ… Published ${exportedCount}/${skillDirs.length} Codex skills in ${targetSkillsDir}`,
    );
}

function generateCodexAgents(sourceRoot, targetDir, useRepoPrefix = true) {
    const sourceAgentsDir = path.join(sourceRoot, KIT_DIR, "agents");
    ...
    const targetAgentsDir = useRepoPrefix
        ? path.join(targetDir, CODEX_AGENTS_DIR)
        : path.join(targetDir, "agents");
    fs.mkdirSync(targetAgentsDir, { recursive: true });
    ...
    for (const fileName of agentFiles) {
        ...
        const sourceContent = fs.readFileSync(sourcePath, "utf8");
        const agentDoc = toCodexAgentToml(sourceContent, fileName);
        const writeResult = writeManagedFile(targetPath, agentDoc);
    ...
    console.log(
        `âœ… Published ${writtenCount}/${agentFiles.length} Codex custom agents in ${targetAgentsDir}`,
    );
}
...
function checkGlobalStatus() {
    ...
    const reportCodexGlobalStatus = () => {
        if (
            fs.existsSync(codexRoot) ||
            fs.existsSync(codexAgentsRoot) ||
            fs.existsSync(codexSkillsRoot)
        ) {
            console.log("Codex global assets installed");
            console.log(`   Instructions: ${countFiles(codexRoot, "AGENTS.md")}`);
            console.log(`   Agents: ${countFiles(codexAgentsRoot, ".toml")}`);
            console.log(`   Skills: ${countDirectories(codexSkillsRoot)}`);
        } else {
            console.log("Codex global assets not installed");
        }
    };

    console.log("📊 my-agents-kit Global Status");
    console.log("");
    reportCodexGlobalStatus();
...
function toCodexAgentToml(sourceContent, fileName) {
    const { frontmatter, body } = parseFrontmatter(sourceContent);
    const name = frontmatter.name || fileName.replace(/\.md$/, "");
    const sourceRelativePath = toPosixPath(path.join(KIT_DIR, "agents", fileName));
    const description =
        frontmatter.description ||
        `Specialized agent generated from ${sourceRelativePath}.`;

    return `# ${GENERATED_BY} from ${sourceRelativePath}. Edit this file or remove this header to manage it manually.
name = ${toTomlString(name)}
description = ${toTomlString(description)}
developer_instructions = ${toTomlString(body.trim())}
`;
}

function toCodexSkillMarkdown(sourceContent, sourceRelativePath) {
    return `${sourceContent.trim()}\n\n<!-- ${GENERATED_BY} from ${sourceRelativePath}. Edit this file or remove this header to manage it manually. -->\n`;
}
...
function copyManagedDirectory(sourceDir, targetDir, sourceRelativeDir) {
    ...
    for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
            copyManagedDirectory(sourcePath, targetPath, sourceRelativePath);
            continue;
        }
        ...
        if (entry.name.endsWith(".md")) {
            const sourceContent = fs.readFileSync(sourcePath, "utf8");
            const managedContent = toCodexSkillMarkdown(
                sourceContent,
                sourceRelativePath,
            );
            writeManagedFile(targetPath, managedContent);
