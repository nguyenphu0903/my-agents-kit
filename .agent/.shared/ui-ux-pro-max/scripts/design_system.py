...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append(f"# Page Design: {page_name.title()}")
    lines.append("")
    lines.append(f"> **LOGIC:** This file OVERRIDES `../MASTER.md` for the {page_name} page only.")
    lines.append(f"> Any rule NOT specified here falls back to MASTER.md.")
    lines.append("")
    lines.append("---")
    lines.append("")
    lines.append(f"**Project:** {project}")
    ...
    lines.append(f"**Generated:** {timestamp}")
    if page_query:
        lines.append(f"**Context:** {page_query}")
    lines.append("")
    lines.append("---")
    lines.append("")
    ...
    lines.append("")
    lines.append("### Goal")
    lines.append("[Define the primary goal of this page]")
    lines.append("")
    lines.append("### Key Elements")
    lines.append("1. [Element 1]")
    lines.append("2. [Element 2]")
    lines.append("3. [Element 3]")
    lines.append("")
    
    lines.append("## Deviations from Master")
    lines.append("")
    lines.append("### Layout Overrides")
    lines.append("- [ ] Override 1")
    lines.append("- [ ] Override 2")
    lines.append("")
    
    lines.append("### Component Specifics")
    lines.append("```css")
    lines.append(f"/* Special styles for {page_name} */")
    lines.append(f".{page_name.lower().replace(' ', '-')}-container {{")
